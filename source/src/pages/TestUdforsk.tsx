...
export default function TestUdforsk() {
  ...
  // Superhjerne: Get trending tags across all events
  const trendingTags = useMemo(() => {
    return tagEngine.getTrendingTags(events, 15);
  }, [events]);

  // Superhjerne: Get related tags when a tag is selected
  ...
  // Superhjerne: Cross-page tag suggestions (firma + henvisning)
  ...
  return (
    <div className="min-h-screen bg-[#0a1929] pb-24">
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-1 bg-white/5 rounded-lg p-1">
            <button
              ...
              className={`p-1.5 rounded ${viewMode === "grid" ? "bg-[#4ECDC4]/20 text-[#4ECDC4]" : "text-white/40"}`}
            >
              <Grid size={16} />
            </button>
            <button
              onClick={() => setViewMode("list")}
              className={`p-1.5 rounded ${viewMode === "list" ? "bg-[#4ECDC4]/20 text-[#4ECDC4]" : "text-white/40"}`}
            >
              <List size={16} />
            </button>
          </div>
      ...
      {/* Superhjerne: Related tags + cross-page links */}
      {selectedTag && (
        <div className="px-4 mb-4">
